...
*ingrdfile*\ [=id[**+s**\ *scale*][**+o**\ *offset*][**+n**\ *invalid*]]
    ...
    若讀入的網格文件不是標準的netCDF格式文件，則需要加上 **=**\ *id* 以指定
    網格文件格式（見 :doc:`/grid/format`\ ）。此外，

    - **+s**\ *scale* 對數據做比例縮放，即將數據乘以 *scale*
    - **+o**\ *offset* 對數據做偏移，即將數據加上 *offset*
    - **+n**\ *invalid* 數據中哪個值表示無效值

    需要注意的是，在讀入網格文件時，總是先縮放再偏移。
...
**-G**\ *outgrdfile*\ [=id[**+s**\ *scale*][**+o**\ *offset*][**+n**\ *invalid*]][*:driver*\ [/*datatype*]]]
    ...
    若要寫的網格文件格式不是標準的netCDF格式，則需要加上 **=**\ *id* 以指定
    網格文件格式（見 :doc:`/grid/format`\ ）。此外：
...
**-N**
    在生成native二進制文件時，不將GMT網格文件頭段寫到文件中。
...
GMT默認只能讀取並處理2D單變量網格。對於多變量、多維度網格文件，需要使用額外的語法
指定要讀取的變量或維度，詳情見 :doc:`/grid/read`\ 。

示例
----

將網格文件轉換成四字節native浮點型網格::

    gmt grdconvert data.nc ras_data.b4=bf -V

將網格文件轉換成二字節短整型文件，將其乘以10並減去32000，並設置無數據節點的值爲-9999::
