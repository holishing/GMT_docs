...
GMT中的某些模塊在對網格文件做某些操作（比如插值或計算偏導）時，在網格邊界處
...
其中 :math:`f(x, y)` 是網格文件內的值，\ :math:`\partial/\partial n` 是垂直於
這個方向的偏導。
...
- 對於網格線配準的網格文件，共N+1列數據。第一列數據位於 :math:`x = x_{min}` 處，
  最後一列（N+1列）數據位於 :math:`x = x_{max}` 處，週期邊界條件意味着數據的
  第一列和最後一列是完全相同的
- 對於像素配準的網格文件，有N列數據，第一列位於 :math:`x_{min} + x_{inc}/2`\ ，
